...
if not redis:get(nk..'group:add'..msg.chat_id_) then return sendMsg(msg.chat_id_,msg.id_,'★︎  المجموعه بالتأكيد تم تعطيلها \n \n') end  
rem_data_group(msg.chat_id_)
sendMsg(msg.chat_id_,msg.id_,' تـم تـعـطـيـل الـمـجـمـوعـه ★︎ \n \n')
end,{msg=msg})
end
...
sendMsg(arg.chat_id_,arg.id_,'★︎  لا يوجد صوره في بروفايلك ... !\n\n'..Flter_Markdown(KleshaID))
end
else
...
if not data.id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لآ يوجد عضـو بهہ‌‏ذآ آلمـعرف \n") end 
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
sendMsg(arg.ChatID,arg.MsgID,"★︎  آضـغط على آلآيدي ليتم آلنسـخ\n\n "..UserName.." ~⪼ ( `"..UserID.."` )")
...
if not msg.SuperCreator  then return "★︎  هذا الامر يخص {المنشئ الاساسي,المطور} فقط  \n" end
...
sendMsg(arg.ChatID,arg.MsgID,"★︎  أهلا عزيزي "..arg.TheRankCmd.." \n** تم تثبيت الرساله \n")
elseif data.ID == "Error" and data.code_ == 6 then
sendMsg(arg.ChatID,arg.MsgID,'★︎  عذرا لا يمكنني التثبيت .\n★︎  لست مشرف او لا املك صلاحيه التثبيت \n ')    
end
...
elseif data.ID == "Error" and data.code_ == 6 then
sendMsg(arg.ChatID,arg.MsgID,'★︎  عذرا لا يمكنني الغاء التثبيت .\n★︎  لست مشرف او لا املك صلاحيه التثبيت \n ')    
elseif data.ID == "Error" and data.code_ == 400 then
sendMsg(arg.ChatID,arg.MsgID,'★︎  عذرا عزيزي '..arg.TheRankCmd..' .\n★︎  لا توجد رساله مثبته لاقوم بازالتها \n ')    
end
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكن تقييد البوت  \n") 
elseif UserID == 1405398498 or UserID == 1399282735 then  
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد مطور السورس\n") 
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد المطور الاساسي\n") 
elseif redis:sismember(nk..':SUDO_BOT:',UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد المطور\n") 
elseif redis:sismember(nk..':MONSHA_BOT:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد المنشئ\n") 
elseif redis:sismember(nk..':MONSHA_Group:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد المنشئ الاساسي\n") 
elseif redis:sismember(nk..'owners:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد المدير\n") 
elseif redis:sismember(nk..'admins:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد الادمن\n") 
elseif  redis:sismember(nk..'whitelist:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد المميز\n") 
end
GetChatMember(arg.ChatID,UserID,function(arg,data)
if data.status_.ID == "ChatMemberStatusMember" then
GetUserID(UserID,function(arg,data)
NameUser = Hyper_Link_Name(data)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم تقييده  من المجموعه") 
...
sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنني تقيد العضو لانه مغادر المجموعة \n") 
else
sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنني تقييد المشرف\n") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
GetUserName(MsgText[2],function(arg,data)
if not data.id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لآ يوجد عضـو بهہ‌‏ذآ آلمـعرف \n") end 
if data.type_.ID == "ChannelChatInfo" then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا معرف قناة وليس حساب \n") end
local UserID = data.id_
NameUser = Hyper_Link_Name(data)
if UserID == our_id then   
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد البوت\n") 
elseif  UserID == 1405398498 or UserID == 1399282735  then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎   لا يمكنك تقييد مطور السورس\n") 
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تقييد المطور الاساسي\n") 
...
Restrict(arg.ChatID,arg.UserID,1)  
redis:set(nk..":TqeedUser:"..arg.ChatID..arg.UserID,true)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..arg.NameUser.." 」 \n★︎  تم تقييده  من المجموعه") 
else
sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني تقييد العضو .\n★︎  لانه مشرف في المجموعه \n ')    
end
end,{ChatID=arg.ChatID,MsgID=arg.MsgID,UserName=arg.UserName,UserID=arg.UserID,NameUser=arg.NameUser})
else
sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني تقييد العضو .\n★︎  لانني لست مشرف في المجموعه \n ')    
end
...
UserID =  MsgText[2] 
if UserID == our_id then   
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك تقييد البوت\n") 
elseif UserID == 1405398498 or UserID == 1399282735 then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك تقييد مطور السورس\n") 
elseif UserID == tostring(SUDO_ID) then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك تقييد المطور الاساسي\n") 
elseif redis:sismember(nk..':SUDO_BOT:',UserID) then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك تقييد المطور\n") 
elseif redis:sismember(nk..':MONSHA_BOT:'..msg.chat_id_,UserID) then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك تقييد المنشئ\n") 
elseif redis:sismember(nk..':MONSHA_Group:'..msg.chat_id_,UserID) then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك نقييد المنشئ الاساسي\n") 
elseif redis:sismember(nk..'owners:'..msg.chat_id_,UserID) then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك تقييد المدير\n") 
elseif redis:sismember(nk..'admins:'..msg.chat_id_,UserID) then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  لا يمكنك تقييد الادمن\n") 
end
GetUserID(MsgText[2],function(arg,data)
if not data.id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لآ يوجد حساب بهذا الايدي  \n") end 
...
GetChatMember(arg.ChatID,our_id,function(arg,data)
if data.status_.ID ~= "ChatMemberStatusEditor" then 
return sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني تقييد العضو .\n★︎  لانني لست مشرف في المجموعه \n ')    
end
Restrict(arg.ChatID,arg.UserID,1)
redis:set(nk..":TqeedUser:"..arg.ChatID..arg.UserID,true)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم تقييده  من المجموعه") 
...
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
if UserID == our_id then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تنفيذ الامر بالرد ع رسالة البوت \n") end
Restrict(arg.ChatID,UserID,2)
redis:del(nk..":TqeedUser:"..arg.ChatID..UserID)
GetUserID(UserID,function(arg,data)
NameUser = Hyper_Link_Name(data)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم فك تقييده  من المجموعه") 
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_}) 
...
return sendMsg(ChatID,MsgID,"★︎  البوت ليس مقييد \n ") 
end
GetChatMember(arg.ChatID,our_id,function(arg,data)
if data.status_.ID ~= "ChatMemberStatusEditor" then 
return sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني قك تقييد العضو .\n★︎  لانني لست مشرف في المجموعه \n ')    
end
redis:del(nk..":TqeedUser:"..arg.ChatID..arg.UserID)
Restrict(arg.ChatID,arg.UserID,2)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم فك تقييده  من المجموعه") 
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد رفعه قرد فالمجموعه") 
else
redis:hset(nk..'username:'..arg.UserID,'username',ReUsername)
redis:sadd(nk..'salem:'..arg.ChatID,arg.UserID)
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم رفعه قرد في المجموعه") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
GetUserName(MsgText[2],function(arg,data)
if not data.id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لآ يوجد عضـو بهہ‌‏ذآ آلمـعرف \n") end 
if data.type_.user_ and data.type_.user_.type_.ID == "UserTypeBot" then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنني رفع حساب بوت \n") end 
local UserID = data.id_
NameUser = Hyper_Link_Name(data)
if UserID == our_id then
return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا لا يمكنني رفع البوت \n") 
elseif data.type_.ID == "ChannelChatInfo" then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا معرف قناة وليس حساب \n") 
end
UserName = arg.UserName
if redis:sismember(nk..'salem:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد رفعه قرد في لمجموعه") 
end
redis:hset(nk..'username:'..UserID,'username',UserName)
redis:sadd(nk..'salem:'..arg.ChatID,UserID)
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم رفعه قرد في الجموعه") 
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
...
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد تنزيله من قائمة القدره") 
else
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد تنزيله من قائمه القرده")
else
...
return "★︎ بواسطه ⇠ "..msg.TheRankCmd.."   \n  تم مسح {* "..kerd.." *} من قائمه القرده  \n"
end
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد رفعه قلبك") 
else
redis:hset(nk..'username:'..arg.UserID,'username',ReUsername)
redis:sadd(nk..'salem1:'..arg.ChatID,arg.UserID)
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم رفعه قلبك فالمجموعه") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد رفعه وتكه") 
else
redis:hset(nk..'username:'..arg.UserID,'username',ReUsername)
redis:sadd(nk..'salem2:'..arg.ChatID,arg.UserID)
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم رفعه وتكه في المجموعه") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد تنزيله من قائمه الوتك") 
else
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد رفعه زوجتك") 
else
redis:hset(nk..'username:'..arg.UserID,'username',ReUsername)
redis:sadd(nk..'salem3:'..arg.ChatID,arg.UserID)
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم رفعها زوجتك فالمجموعه ارفع راسنا") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد رفعها زوجتك") 
end
redis:hset(nk..'username:'..UserID,'username',UserName)
redis:sadd(nk..'salem3:'..arg.ChatID,UserID)
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم رفعها زوجتك ارفع راسنا") 
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
GetUserID(MsgText[2],action_by_id,{msg=msg,cmd="raf3salem1"})
end 
...
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد تنزيل زوجتك") 
else
redis:srem(nk..'salem3:'..arg.ChatID,arg.UserID)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم تنزيلها من قائمه زوجاتك") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد تنزيلها من قائمه زوجاتك")
else
...
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎ تم بالتأكيد تنزيله من قائمه ازواجك") 
else
...
if not msg.Creator then return "★︎  هذا الامر يخص {المطور,المنشئ} فقط  \n" end
if not MsgText[2] and msg.reply_id then
GetMsgInfo(msg.chat_id_,msg.reply_id,function(arg,data)
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
...
if not msg.SuperCreator then return "★︎  هذا الامر يخص {المطور,المطور الاساسي} فقط  \n" end
if not MsgText[2] and msg.reply_id then
GetMsgInfo(msg.chat_id_,msg.reply_id,function(arg,data)
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
...
if not data.sender_user_id_ then return sendMsg(ChatID,MsgID,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
if UserID == our_id then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تنفيذ الامر بالرد ع رسالة البوت \n") end
GetUserID(UserID,function(arg,data)
USERNAME = ResolveUserName(data):gsub([[\_]],"_")
...
if not msg.Director then return "★︎  هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n" end
if not MsgText[2] and msg.reply_id then
GetMsgInfo(msg.chat_id_,msg.reply_id,function(arg,data)
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
...
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'★︎  الاسم ⇠ '..namei..'\n'
..'★︎  الايدي ⇠ {'..arg.UserID..'} \n'
..'★︎  المعرف ⇠ '..useri..'\n'
..'★︎  الرتبه ⇠ '..Getrtba(arg.UserID,arg.ChatID)..'\n'
..'★︎  نوع الكشف ⇠ بالرد\n',13,utf8.len(namei))
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
elseif MsgText[2] and MsgText[2]:match('@[%a%d_]+') then
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
sendMsg(arg.ChatID,arg.MsgID,'*★︎ * الاسم ⇠ '..FlterName(data.title_,30)..'\n'..'★︎  الايدي ⇠ {`'..UserID..'`} \n'..'★︎  المعرف ⇠ '..UserName..'\n★︎  الرتبه ⇠ '..Getrtba(UserID,arg.ChatID)..'\n*★︎ * نوع الكشف ⇠ بالمعرف\n'..'')
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
...
if not msg.Admin then return "هذا الامر ليس لك عزيزي .  \n" end
if not msg.Creator and not redis:get(nk.."lock_KickBan"..msg.chat_id_) then return "★︎  الامر معطل من قبل اداره المجموعة  \n" end
...
local UserID = data.sender_user_id_
if UserID == our_id then   
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد البوت\n") 
elseif UserID == 1405398498 or UserID == 1399282735 then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد مطور السورس\n") 
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد المطور الاساسي\n") 
elseif redis:sismember(nk..':SUDO_BOT:',UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد المطور\n") 
elseif redis:sismember(nk..':MONSHA_BOT:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد المنشئ\n") 
elseif redis:sismember(nk..':MONSHA_Group:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد المنشئ الاساسي\n") 
elseif redis:sismember(nk..'owners:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد المدير\n") 
elseif redis:sismember(nk..'admins:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد الادمن\n") 
elseif  redis:sismember(nk..'whitelist:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك طرد المميز\n") 
end
kick_user(UserID,arg.ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني حظر العضو .\n★︎  لانه مشرف في المجموعه \n ')    
elseif data.ID == "Error" and data.code_ == 3 then
return sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني حظر العضو .\n★︎  ليس لدي صلاحيه الحظر او لست مشرف\n ')    
end
GetUserID(arg.UserID,function(arg,data)
NameUser = Hyper_Link_Name(data)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم طرده  من المجموعه") 
StatusLeft(arg.ChatID,arg.UserID)
end,{ChatID=arg.ChatID,UserID=arg.UserID,MsgID=arg.MsgID})
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
kick_user(UserID,arg.ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني طرد العضو .\n★︎  لانه مشرف في المجموعه \n ')    
elseif data.ID == "Error" and data.code_ == 3 then
return sendMsg(arg.ChatID,arg.MsgID,'★︎  لا يمكنني طرد العضو .\n★︎  ليس لدي صلاحيه الحظر او لست مشرف\n ')    
end
StatusLeft(arg.ChatID,arg.UserID)
sendMsg(arg.ChatID,arg.MsgID,"★︎ العضو 「 "..arg.NameUser.." 」 \n★︎  تم طرده  من المجموعه") 
...
if UserID == our_id then   
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر البوت\n") 
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر المطور الاساسي\n") 
elseif UserID == 1405398498 or UserID == 1399282735 then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر مطور السورس\n") 
elseif redis:sismember(nk..':SUDO_BOT:',UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر المطور\n") 
elseif redis:sismember(nk..':MONSHA_BOT:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر المنشئ\n") 
elseif redis:sismember(nk..':MONSHA_Group:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر المنشئ الاساسي\n") 
elseif redis:sismember(nk..'owners:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر المدير\n") 
elseif redis:sismember(nk..'admins:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر الادمن\n") 
elseif  redis:sismember(nk..'whitelist:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك حظر المميز\n") 
end
...
if not msg.SuperCreator then return "★︎  هذا الامر يخص {منشئ اساسي,المطور} فقط  \n" end
...
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
if UserID == our_id then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكن تنفيذ الامر للبوت\n") end
GetUserID(UserID,function(arg,data)
NameUser = Hyper_Link_Name(data)
if data.type_.ID == "ChannelChatInfo" then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا معرف قناة وليس حساب \n") end
ResAdmin = UploadAdmin(arg.ChatID,arg.UserID,"")  
if ResAdmin == '{"ok":false,"error_code":400,"description":"Bad Request: CHAT_ADMIN_REQUIRED"}' then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنني تنزيله لانه مرفوع من قبل منشئ اخر \n")  end
redis:srem(nk..':MONSHA_BOT:'..arg.ChatID,arg.UserID)
redis:srem(nk..'owners:'..arg.ChatID,arg.UserID)
redis:srem(nk..'admins:'..arg.ChatID,arg.UserID)
redis:srem(nk..'whitelist:'..arg.ChatID,arg.UserID)
...
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
if UserID == our_id then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك استخدام الامر بالرد على البوت \n") end
GetUserID(UserID,function(arg,data)
USERNAME = ResolveUserName(data)
NameUser = Hyper_Link_Name(data)
...
sendMsg(arg.ChatID,arg.MsgID,"★︎  عذرا البوت ليس لديه صلاحيات الحظر \n")
else
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم بالتأكيد الغاء حظره  من المجموعه") 
end
end,{ChatID=arg.ChatID,UserID=arg.UserID,MsgID=arg.MsgID,USERNAME=arg.USERNAME})
redis:srem(nk..'banned:'..arg.ChatID,arg.UserID)
else
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم الغاء حظره  من المجموعه") 
end
...
GetUserName(MsgText[2],function(arg,data)
if not data.id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لآ يوجد عضـو بهہ‌‏ذآ آلمـعرف \n") end 
if data.id_ == our_id then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك تنفيذ الامر مع البوت \n") end 
local UserID = data.id_
UserName = arg.UserName
NameUser = Hyper_Link_Name(data)
...
local UserID = data.sender_user_id_
if UserID == our_id then   
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم البوت\n") 
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم المطور الاساسي\n") 
elseif UserID == 1405398498 or UserID == 1399282735 then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم مطور السورس\n") 
elseif redis:sismember(nk..':SUDO_BOT:',UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم المطور\n") 
elseif redis:sismember(nk..':MONSHA_BOT:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم المنشئ\n") 
elseif redis:sismember(nk..':MONSHA_Group:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم المنشئ الاساسي\n") 
elseif redis:sismember(nk..'owners:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم المدير\n") 
elseif redis:sismember(nk..'admins:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم الادمن\n") 
elseif  redis:sismember(nk..'whitelist:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"★︎  لا يمكنك كتم المميز\n") 
end
GetUserID(UserID,function(arg,data)
USERNAME = ResolveUserName(data)
NameUser = Hyper_Link_Name(data)
if redis:sismember(nk..'is_silent_users:'..arg.ChatID,arg.UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم بالتأكيد كتمه  من المجموعه") 
else
redis:hset(nk..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(nk..'is_silent_users:'..arg.ChatID,arg.UserID)
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم كتمه  من المجموعه") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم بالتأكيد الغاء كتمه  من المجموعه") 
else
sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم الغاء كتمه  من المجموعه") 
redis:srem(nk..'is_silent_users:'..arg.ChatID,arg.UserID)
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
if not msg.Creator  then return "★︎  هذا الامر يخص {المطور,المنشئ الاساسي ,المنشئ} فقط  \n" end 
redis:setex(nk..'WiCmdLink'..msg.chat_id_..msg.sender_user_id_,500,true)
return '★︎  حسننا عزيزي  \n★︎  الان ارسل  رابط مجموعتك '
end
...
return sendMsg(msg.chat_id_,msg.id_,"★︎  تم انشاء رابط جديد \n★︎ ["..LinkGp.."]\n★︎ لعرض الرابط ارسل { الرابط } \n")
else
return sendMsg(msg.chat_id_,msg.id_," لا يمكنني انشاء رابط للمجموعه .\n★︎  لانني لست مشرف في المجموعه \n ")
end
else
return sendMsg(msg.chat_id_,msg.id_," لقد قمت بانشاء الرابط سابقا .\n★︎  ارسل { الرابط } لرؤيه الرابط  \n ")
end
return false
end 

if MsgText[1] == "الرابط" then
if not redis:get(nk.."lock_linkk"..msg.chat_id_) then return "★︎  الامر معطل من قبل الادارة \n^"  end
if not redis:get(nk..'linkGroup'..msg.chat_id_) then return "★︎   لا يوجد رابط\n★︎ لانشاء رابط ارسل { انشاء رابط } \n" end
local GroupName = redis:get(nk..'group:name'..msg.chat_id_)
local GroupLink = redis:get(nk..'linkGroup'..msg.chat_id_)
return "★︎  رابـط الـمـجـمـوعه :\n\n["..GroupLink.."]\n"
end

if MsgText[1] == "ضع القوانين" then
if not msg.Admin then return "هذا الامر ليس لك عزيزي .  \n" end
redis:setex(nk..'rulse:witting'..msg.chat_id_..msg.sender_user_id_,300,true)
return '★︎  حسننا عزيزي  \n★︎  الان ارسل القوانين  للمجموعه '
end

if MsgText[1] == "القوانين" then
if not redis:get(nk..'rulse:msg'..msg.chat_id_) then 
return "★︎  مرحبأ عزيري ★︎  القوانين كلاتي ★︎ \n★︎ ممنوع نشر الروابط \n★︎ ممنوع التكلم في الدين او السياسة او نشر صور اباحيه \n★︎ ممنوع  اعاده توجيه\n★︎ ممنوع التكلم بلطائفه \n★︎ الرجاء احترام المدراء والادمنيه\n"
else 
...
return "★︎  حدود التكرار ,  يجب ان تكون ما بين  *[2-50]*" 
end
redis:set(nk..'num_msg_max'..msg.chat_id_,MsgText[2]) 
return "★︎  تم وضع التكرار ⇠ { *"..MsgText[2].."* }"
end

if MsgText[1] == "ضع وقت التنظيف" then
if not msg.Creator then return "★︎  هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n" end
local NumLoop = tonumber(MsgText[2])
redis:set(nk..':Timer_Cleaner:'..msg.chat_id_,NumLoop) 
return "★︎  تم وضع وقت التنظيف ⇠ { *"..MsgText[2].."* } ساعه"
end
...
if Mn3Word == 0 then 
return "★︎  عذرا لا توجد كلمات ممنوعه ليتم حذفها " 
end
redis:del(nk..':Filter_Word:'..msg.chat_id_)
return "★︎ بواسطه ⇠ "..msg.TheRankCmd.."   \n★︎ تم مسح {*"..Mn3Word.."*} كلمات من المنع "
end
...
if not msg.SuperCreator    then return "★︎  هذا الامر يخص {المطور,منشئ الاساسي} فقط  \n" end
...
if MKTOMEN ==0 then 
return "★︎  لا يوجد مستخدمين مكتومين في المجموعه " 
end
...
if not msg.Director then return "★︎  هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n" end
if 500 < tonumber(MsgText[2]) then return "★︎  حدود المسح ,  يجب ان تكون ما بين  *[2-500]*" end
...
sendMsg(arg.ChatID,arg.MsgID,'★︎  عذرا , لا توجد صوره في المجموعة\n')
elseif data.message_ and data.message_ == "CHAT_ADMIN_REQUIRED" then
sendMsg(arg.ChatID,arg.MsgID,'★︎  عذرا , البوت ليس لدية صلاحيه التعديل في المجموعة \n')
else
sendMsg(arg.ChatID,arg.MsgID,'★︎  تم حذف صوره آلمـجمـوعهہ \n')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
return false
end
...
return '★︎  حسننا عزيزي ★︎ \n★︎  الان قم بارسال الصوره\n' 
end 
end
...
if not msg.Director  then return "★︎  هذا الامر يخص {المدير,المنشئ,المطور} فقط  \n" end
...
if not msg.Director then return "★︎  هذا الامر يخص {المطور,المنشئ} فقط  \n" end
...
if Total == 1 then
return sendMsg(arg.ChatID,arg.MsgID,"★︎ لا يـوجـد بـوتـات في الـمـجـمـوعـه .") 
else
local NumBot = 0
local NumBotAdmin = 0
...
if NumBot == 0 then 
TextR = TextR.."★︎ لا يـمـكـن طردهم لانـهـم مشـرفـين .\n"
else
if NumBotAdmin >= 1 then
TextR = TextR.."★︎ لم يتم طـرد {* "..NumBotAdmin.." *} بوت لآنهہ‌‏م مـشـرفين."
else
TextR = TextR.."★︎ تم طـرد كــل البوتآت بنجآح .\n"
end
end
...
local total = data.total_count_ or 0
AllBots = '★︎  قـائمه البوتات الـحالية\n\n'
local NumBot = 0
for k, v in pairs(data.members_) do
GetUserID(v.user_id_,function(arg,data)
...
★︎ لـديـک {]]..total..[[} بـوتـآت
★︎ ملاحظة : الـ ★ تعنـي ان البوت مشرف في المجموعـة.]]
...
USERNAME = ResolveUserName(data)
USERCAR  = utf8.len(USERNAME)
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'★︎ العضو ⇠ '..USERNAME..'\n\nـ⠀•⊱ { رتـبـه الشخص } ⊰•\n\n★︎  في البوت ⇠ '..arg.Getrtb..' \n★︎ في المجموعه ⇠ '..arg.SudoGroups..'\n',12,utf8.len(USERNAME)) 
...
if data.status_.ID ~= "ChatMemberStatusMember" then 
sendMsg(arg.ChatID,arg.MsgID,'★︎  جيد , الـبــوت ادمــن الان \n')
else 
sendMsg(arg.ChatID,arg.MsgID,'★︎  كلا البوت ليس ادمن في المجموعة ')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
return false 
end
...
.."★︎ ¦ الـجـهـات •⊱ { `"..NumGha.."` } ⊰•\n"
.."★︎ ¦ الـصـور •⊱ { `"..photo.."` } ⊰•\n"
.."★︎ ¦ الـمـتـحـركـه •⊱ { `"..animation.."` } ⊰•\n"
.."★︎  الـمـلـصـقات •⊱ { `"..sticker.."` } ⊰•\n"
.."★︎ ¦ الـبـصـمـات •⊱ { `"..voice.."` } ⊰•\n"
.."★︎ ¦ الـصـوت •⊱ { `"..audio.."` } ⊰•\n"
.."★︎ ¦ الـفـيـديـو •⊱ { `"..video.."` } ⊰•\n"
.."★︎  الـتـعـديـل •⊱ { `"..edited.."` } ⊰•\n\n"
.."★︎ ¦ تـفـاعـلـك  •⊱ "..Get_Ttl(msgs).." ⊰•\n"
.."ـ.——————————\n"
...
return '★︎   عدد جهاتك يبشه‏‏ ⇠ 【'..(redis:get(nk..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..'】 . \n'
end
...
if data.username_ then USERNAME = "★︎ ¦ المعرف •⊱ @["..data.username_.."] ⊰•\n" end 
...
local Get_info = "★︎  اهـلا بـك عزيزي في معلوماتك  \n"
.."ـ.——————————\n"
.."★︎  الاســم •⊱{ "..FlterName(Name,25) .." }⊰•\n"
..USERNAME
.."★︎ ¦ الايـدي •⊱ { `"..data.id_.."` } ⊰•\n"
.."★︎  رتبتــك •⊱ "..arg.TheRank.." ⊰•\n"
.."★︎ ¦ ــ •⊱ { `"..arg.chat_id_.."` } ⊰•\n"
.."ـ.——————————\n"
.." •⊱ { الاحـصـائـيـات الـرسـائـل } ⊰•\n"
.."★︎  الـرسـائـل •⊱ { `"..msgs.."` } ⊰•\n"
...
return "★︎  أهلا عزيزي "..msg.TheRankCmd.."  \n★︎  نورت المجموعه \n" 
end 
end
...
return "★︎  تم مسح {* "..mtwren.." *} من المطورين \n"
end
...
return "★︎  تـم مـسـح { *"..addbannds.." *} من قائمه العام\n" 
end 
...
local UserID = data.sender_user_id_
if UserID == our_id then 
return sendMsg(ChatID,MsgID,"★︎  عذرا لا يمكنني رفع بوت \n") 
end
GetUserID(UserID,function(arg,data)
ReUsername = ResolveUserName(data)
NameUser = Hyper_Link_Name(data)
...
if not msg.SudoBase then return "★︎  هذا الامر يخص {مطور اساسي} فقط  \n" end
...
sendMsg(msg.chat_id_,msg.id_,"★︎  تم مسح كلايش التعليمات  \n")
end

if MsgText[1] == 'مسح كليشه الايدي' or MsgText[1] == 'مسح الايدي' or MsgText[1] == 'مسح ايدي'  or MsgText[1] == 'مسح كليشة الايدي'  then 
if not msg.Creator then return "★︎  هذا الامر يخص {منشئ اساسي,المنشئ,المطور} فقط  \n" end
redis:del(nk..":infoiduser_public:"..msg.chat_id_)
sendMsg(msg.chat_id_,msg.id_,"★︎  تم مسح كليشة الايدي بنجاح \n")
end
...
if not MsgText[2] and msg.reply_id then 
GetMsgInfo(msg.chat_id_,msg.reply_id,function(arg,data)
if not data.sender_user_id_ then return sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا هذا العضو ليس موجود ضمن المجموعات \n") end
local UserID = data.sender_user_id_
msg = arg.msg
msg.UserID = UserID
GetUserID(UserID,function(arg,data)
...
msg = arg.msg
UserID = msg.UserID
if UserID == our_id then return sendMsg(msg.chat_id_,msg.id_,"★︎  لآ يمكنك تنفيذ الامر مع البوت\n") end
if UserID == 1405398498 or UserID == 1399282735 then return sendMsg(msg.chat_id_,msg.id_,"★︎  لآ يمكنك تنفيذ الامر ضد مطور السورس \n") end
...
DonisDown = DonisDown.."★︎  تم تنزيله من المطور ️\n"
end 
if redis:sismember(nk..':MONSHA_Group:'..msg.chat_id_,UserID) then 
DonisDown = DonisDown.."★︎  تم تنزيله من المنشئ الاساسي ️\n"
end 
if redis:sismember(nk..':MONSHA_BOT:'..msg.chat_id_,UserID) then 
DonisDown = DonisDown.."★︎  تم تنزيله من المنشئ ️\n"
end 
if redis:sismember(nk..'owners:'..msg.chat_id_,UserID) then 
DonisDown = DonisDown.."★︎  تم تنزيله من المدير ️\n"
end 
if redis:sismember(nk..'admins:'..msg.chat_id_,UserID) then 
DonisDown = DonisDown.."★︎  تم تنزيله من الادمن ️\n"
end 
if redis:sismember(nk..'whitelist:'..msg.chat_id_,UserID) then
DonisDown = DonisDown.."★︎  تم تنزيله من العضو مميز ️\n"
end
function senddwon() sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا المستخدم رتبته اعلى منك لا يمكن تنزيله \n") end
function sendpluse() sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا لا يمكن تنزيل رتبه مثل رتبتك : "..msg.TheRankCmd.." \n") end

if rinkuser == 8 then return sendMsg(msg.chat_id_,msg.id_,"★︎ المستخدم  ⇠「 "..NameUser.." 」   \nانه بالتأكيد عضو \n️")  end
huk = false
...
if not huk then sendMsg(msg.chat_id_,msg.id_,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n"..DonisDown.."\n️") end
...
if not data.id_ then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لآ يوجد عضـو بهہ‌‏ذآ آلمـعرف \n") end 
local UserID = data.id_
if UserID == our_id then return sendMsg(arg.ChatID,arg.MsgID,"★︎  لآ يمكنك تنفيذ الامر مع البوت\n") end
...
message = "★︎ الاوامر الجديد : \n\n" i = 0
for name,Course in pairs(list) do i = i + 1 message = message ..i..' - *{* '..name..' *}* ~> '..Course..' \n'  end 
if i == 0 then return "★︎  لا توجد اوامر مضافه في القائمه \n " end
return message
end
...
if Awammer ~= 0 then
return "★︎  تم مسح قائمه الاوامر \n..."
else
return "★︎  القائمه بالفعل ممسوحه \n"
end
end
...
return "★︎  حسننا عزيزي , لتغير امر {* "..MsgText[2].." *}  ارسل الامر الجديد الان \n..."
else
return "★︎  عذرا لا يوجد هذا الامر في البوت لتتمكن من تغييره  \n"
end
else
redis:setex(nk..":Witting_changeamr2:"..msg.chat_id_..msg.sender_user_id_,300,true)
return "★︎  حسننا عزيزي , لتغير امر  ارسل الامر القديم الان \n..."
end
end
...
if checkk ~=0 then
return "★︎  تم مسح الامر {* "..MsgText[2].." *} من قائمه الاومر \n..."
else
return "★︎  هذا الامر ليس موجود ضمن الاوامر المضافه  \n"
end
else
redis:setex(nk..":Witting_AmrDel:"..msg.chat_id_..msg.sender_user_id_,300,true)
return "★︎   ارسل الامر الجديد المضاف بالقوائم الان\n..."
end
...
return "★︎  حسننا عزيزي  \n★︎  الان ارسل معرف المستخدم لنقل ملكية البوت له ."
end
...
return'★︎  حسننا عزيزي ★︎ \n★︎  الان قم بارسال الصوره للترحيب \n' 
end
...
sendPhoto(msg.chat_id_,msg.id_,Photo_Weloame,[[★︎  مـرحبا أنا بوت آسـمـي ]]..redis:get(nk..':NameBot:')..[[ 🎖
★︎  يمكنني حمايه المجموعات من السبام والتوجيه الخ....
★︎ [قـناة الـسورس](HTTPS://T.ME/TH3NK) 
]]..SUDO_USERR) 
return false
else
...
return '★︎  حسننا عزيزي ★︎ \n★︎ الان قم بارسال الكليشه \n' 
end

if MsgText[1] == "ضع شرط التفعيل" and MsgText[2] and MsgText[2]:match('^%d+$') then 
redis:set(nk..':addnumberusers',MsgText[2]) 
return '💱 تم وضـع شـرط آلتفعيل آلبوت آذآ كآنت آلمـجمـوعهہ‏‏ آكثر مـن *【'..MsgText[2]..'】* عضـو  ★︎ \n' 
end

if MsgText[1] == "شرط التفعيل" then 
return'★︎  شـرط آلتفعيل آلبوت آذآ كآنت آلمـجمـوعهہ‏‏ آكثر مـن *【'..redis:get(nk..':addnumberusers')..'】* عضـو  ★︎ \n' 
end 
end
...
if not msg.SudoUser then return "★︎  هذا الامر يخص {المطور} فقط  \n" end
if not msg.SudoBase and not redis:get(nk.."lockidedit") then return "★︎  الامر معطل من قبل المطور الاساسي  \n" end
redis:del(nk..":infoiduser")
return sendMsg(msg.chat_id_,msg.id_,"★︎  تم مسح كليشة الايدي العام بنجاح \n")
end
...
if not msg.SudoUser then return "★︎  هذا الامر يخص {المطور} فقط  \n" end
if not msg.SudoBase and not redis:get(nk.."lockidedit") then return "★︎  تعيين الايدي معطل من قبل المطور الاساسي  \n" end
...
if not msg.SudoBase then return "★︎  هذا الامر يخص {المطور} فقط  \n" end
...
sendMsg(arg.chat_id_,arg.id_,'★︎  البوت ليس بالمجموعة ولكن تم مسح بياناتها \n المجموعةة ⇠ ['..arg.name_gp..']\n★︎  الايدي ⇠ ( *'..arg.Group..'* )\n')
else 
sendMsg(arg.chat_id_,arg.id_,'★︎  البوت ليس مفعل بالمجموعه \n ولا يوجد بيانات لها ️')
end
else
StatusLeft(arg.Group,our_id)
if redis:sismember(nk..'group:ids',arg.Group) then
sendMsg(arg.Group,0,'★︎  تم تعطيل المجموعه بأمر من المطور  \n★︎  سوف اغادر جاوو ...\n★︎')
rem_data_group(arg.Group)
sendMsg(arg.chat_id_,arg.id_,'★︎  تم تعطيل المجموعه ومغادرتها \n المجموعةة ⇠ ['..arg.name_gp..']\n★︎  الايدي ⇠ ( *'..arg.Group..'* )\n')
else 
sendMsg(arg.chat_id_,arg.id_,'★︎  البوت ليس مفعل بالمجموعة \n★︎  ولكن تم مغادرتها\n المجموعةة ⇠ ['..arg.name_gp..']\n')
end
end 
...
return "★︎  حسننا الان ارسل رساله ليتم اذاعتها بالتثبيت  \n" 
end
...
return "★︎  الاذاعه مقفوله من قبل المطور الاساسي  " 
end
redis:setex(nk..'fwd:'..msg.sender_user_id_,300, true) 
return "★︎  حسننا الان ارسل التوجيه للاذاعه \n" 
end
...
return "★︎  حسننا الان ارسل الكليشه للاذاعه عام \n" 
end
...
return "★︎  حسننا الان ارسل الكليشه للاذاعه خاص \n"
end
...
return "★︎  حسننا الان ارسل الكليشه للاذاعه للمجموعات \n" 
end
...
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم بالتأكيد الغاء حظره العام  من المجموعات \n") 
else
return sendMsg(arg.ChatID,arg.MsgID,"★︎ المستخدم  ⇠「 "..NameUser.." 」 \n★︎  تم الغاء حظره العام  من المجموعات \n") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.reply_id})
end
...
if MsgText[1] == "رتبتي" then return '★︎  رتبتك ⇠ '..msg.TheRank..'\n' end
...
return '★︎  تم آلغآء آلآمـر بنجآح \n'
end  
...
sendMsg(msg.chat_id_,msg.id_,'★︎  يوجد تحديث جديد الان \n★︎  جاري تنزيل وتثبيت التحديث  ...')
...
sendMsg(arg.chat_id_,arg.id_,"★︎  عذرا النسخه الاحتياطيه هذا ليست للبوت ⇠ ["..Bot_User.."]  \n")
end
else 
sendMsg(arg.chat_id_,arg.id_,'★︎  عذرا اسم الملف غير مدعوم للنظام او لا يتوافق مع سورس النيزك يرجى جلب الملف الاصلي الذي قمت بسحبه وبدون تعديل ع الاسم\n')
end  
else
sendMsg(arg.chat_id_,arg.id_,'★︎  عذرا الملف ليس بصيغه Json !?\n')
end 
else
sendMsg(arg.chat_id_,arg.id_,'★︎  عذرا هذا ليس ملف النسحه الاحتياطيه للمجموعات\n')
end 
end,{chat_id_=msg.chat_id_,id_=msg.id_})
else 
...
return '★︎  لا يوجد ردود ليتم مسحها \n'
end
end
...
return "★︎  حسننا عزيزي  \n★︎  الان ارسل الرد لمسحها من  للمجموعه "
end
...
return '★︎  لا يوجد ردود مضافه حاليا \n' 
end
local ii = 1
local message = '★︎  ردود البوت في المجموعه  :\n\n'
...
local ii = 1
local message = '★︎  الردود العامه في البوت :   :\n\n'
for i=1, #photo 	do message = message ..ii..' - *{* '..	photo[i]..' *}_*( صوره ) \n' 	ii = ii + 1 end
for i=1, #names 	do message = message ..ii..' - *{* '..	names[i]..' *}_*( نص ) \n'  	ii = ii + 1 end
for i=1, #voice 	do message = message ..ii..' - *{* '..	voice[i]..' *}_*( بصمه ) \n' 	ii = ii + 1 end
for i=1, #imation 	do message = message ..ii..' - *{* '..imation[i]..' *}_*( متحركه ) \n'ii = ii + 1 end
...
return "★︎  حسننا ,  الان ارسل كلمه الرد للعشوائي \n-"
end
...
return "★︎  حسننا عزيزي  \n★︎  الان ارسل الرد العشوائي لمسحها "
end


if MsgText[1] == "مسح الردود العشوائيه" then
if not msg.Director then return "★︎  هذا الامر يخص {المطور,المنشئ,المدير} فقط  " end
local AlRdod = redis:smembers(nk..':KlmatRRandom:'..msg.chat_id_) 
if #AlRdod == 0 then return "★︎  الردود العشوائيه محذوفه بالفعل\n" end
...
return "★︎  أهلا عزيزي "..msg.TheRankCmd.."  \n★︎  تم مسح جميع الردود العشوائيه\n"
end

if MsgText[1] == "الردود العشوائيه" then
if not msg.Director then return "★︎  هذا الامر يخص {المطور,المنشئ,المدير} فقط  " end
message = "★︎  الردود العشـوائيه :\n\n"
local AlRdod = redis:smembers(nk..':KlmatRRandom:'..msg.chat_id_) 
if #AlRdod == 0 then 
...
return "★︎  حسننا ,  الان ارسل كلمه الرد للعشوائي العام \n-"
end
...
return "★︎  حسننا عزيزي  \n★︎  الان ارسل الرد العشوائي العام لمسحها "
end
...
if not msg.SudoUser then return "★︎  هذا الامر يخص {المطور} فقط  \n" end
message = "★︎  الردود العشـوائيه العام :\n\n"
local AlRdod = redis:smembers(nk..':KlmatRRandom:') 
if #AlRdod == 0 then 
message = message .." لا توجد ردود عشوائيه مضافه !\n"
else
...
return "★︎  حسننا , الان ارسل كلمه الرد \n-"
end
...
sendMsg(msg.chat_id_,msg.id_,"★︎ حسننا لتعيين كليشة الـ *"..MsgText[1].."* \n★︎ ارسل الكليشه الجديده الان \n\n علما يمكنك استخدام الاختصارات الاتي : \n \n#name : لوضع اسم المستخدم\n#id : لوضع ايدي المستخدم\n#username : لوضع معرف المستخدم \n#stast : لوضع نوع رتبه المستخدم \n#game : لوضع تفاعل المستخدم \n#msgs : لاضهار عدد الرسائل \n#auto : لاضهار عدد النقاط \n#edit : لاضهار عدد السحكات \n#bot : لاضهار اسم البوت\n{المطور} : لاضهار معرف المطور الاساسي\n➼")
return false
end
...
if msg.Admin then return "★︎  لا استطيع طرد المدراء والادمنيه والمنشئين  \n" end
if not redis:get(nk.."lock_leftgroup"..msg.chat_id_) then  return "★︎  امر المغادره معطل من قبل اداره المجموعة  \n" end
kick_user(msg.sender_user_id_,msg.chat_id_,function(arg,data)
if data.ID == "Ok" then
StatusLeft(msg.chat_id_,msg.sender_user_id_)
send_msg(msg.sender_user_id_," اهلا عزيزي , لقد تم طردك من المجموعه بامر منك \n★︎ اذا كان هذا بالخطأ او اردت الرجوع للمجموعه \n\n★︎فهذا رابط المجموعه \n★︎ "..Flter_Markdown(redis:get(nk..'group:name'..msg.chat_id_)).." :\n\n["..redis:get(nk..'linkGroup'..msg.chat_id_).."]\n")
sendMsg(msg.chat_id_,msg.id_,"★︎  لقد تم طردك بنجاح , ارسلت لك رابط المجموعه في الخاص اذا وصلت لك تستطيع الرجوع متى شئت ")
else
sendMsg(msg.chat_id_,msg.id_,"★︎  لا استطيع طردك لانك مشرف في المجموعه  ")
end
end)
return false
end
...
if zh_Text then
local url , res = https.request('https://ali-apii.ml/api/zhrfa_nizk.php?text='..zh_Text:gsub(" ",'+'))
...
return "★︎ الاشتراك الاجباري بالفعل معطل . \n"
end
end
...
return "★︎ لا يوجد قناة مفعله ع الاشتراك الاجباري. \n"
end
end
...
if msg.SudoBase then
local text = '★︎  آهہ‏‏لآ عزيزي آلمـطـور \n★︎  آنتهہ‏‏ آلمـطـور آلآسـآسـي هہ‏‏نآ \n...\n\n★︎  تسـتطـيع‏‏ آلتحكم بكل آلآوآمـر آلمـمـوجودهہ‏‏ بآلكيبورد\n★︎  فقط آضـغط ع آلآمـر آلذي تريد تنفيذهہ‏‏'
...
SendMention(arg.sender_user_id_,data.id_,arg.id_,"★︎  تم ارسال الرسالة \n★︎  آلى : "..USERNAME.." ",39,utf8.len(USERNAME)) 
...
sendMsg(msg.chat_id_,msg.id_,"★︎  تـم إرسالـ رسـالتـك\n★︎  ["..SUDO_USERR.."]")
...
redis:del(nk..'addrdRandom1Public:'..msg.chat_id_..msg.sender_user_id_)
redis:del(nk..'addrdRandomPublic:'..msg.chat_id_..msg.sender_user_id_)
sendMsg(msg.chat_id_,msg.id_,'★︎  وصلت الحد الاقصى لعدد الردود \n★︎ ¦ تم اضافه الرد (['..klma..']) للردود العشوائيه .')
redis:del(nk..'replay1RandomPublic'..msg.chat_id_..msg.sender_user_id_)
return false
end
if msg.text then 
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم ادراج صور للرد باقي '..CountRdod..' \n★︎  ارسل رد اخر او ارسل {تم} .')
elseif msg.content_.ID == "MessageVoice" then
redis:sadd(nk..':KlmatRRandom:',klma) 
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم ادراج البصمه للرد باقي '..CountRdod..' \n★︎   ارسل رد اخر او ارسل {تم}')
elseif msg.content_.ID == "MessageAnimation" then
redis:sadd(nk..':KlmatRRandom:',klma) 
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم ادراج المتحركه للرد باقي '..CountRdod..' \n★︎  ارسل رد اخر او ارسل {تم} .')
elseif msg.content_.ID == "MessageVideo" then
redis:sadd(nk..':KlmatRRandom:',klma) 
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم ادراج الفيديو للرد باقي '..CountRdod..' \n★︎  ارسل رد اخر او ارسل {تم} .')
elseif msg.content_.ID == "MessageAudio" then
redis:sadd(nk..':KlmatRRandom:',klma) 
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم ادراج الصوت للرد باقي '..CountRdod..' \n★︎  ارسل رد اخر او ارسل {تم} .')
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم ادراج الملف للرد باقي '..CountRdod..' \n★︎  ارسل رد اخر او ارسل {تم} .')  
elseif msg.content_.ID == "MessageSticker" then
redis:sadd(nk..':KlmatRRandom:',klma) 
...
return sendMsg(msg.chat_id_,msg.id_,'★︎ تم ادراج الملصق للرد باقي '..CountRdod..' \n★︎ ارسل رد اخر او ارسل {تم} .')
end  
...
sendMsg(msg.chat_id_,msg.id_,'♻ تم اضافه رد متعدد عشوائي بنجاح \n★︎ ¦ يمكنك ارسال (['..klma..']) لاظهار الردود العشوائيه .')
redis:del(nk..'replay1Random'..msg.chat_id_..msg.sender_user_id_)
return false
end
...
redis:del(nk..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه صوره للرد بنجاح \n★︎  يمكنك ارسال (['..klma..']) لاضهار الصوره الاتيه .')
elseif msg.content_.ID == "MessageVoice" then
redis:hset(nk..'replay_voice:group:'..msg.chat_id_,klma,msg.content_.voice_.voice_.persistent_id_)
redis:del(nk..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه بصمه صوت للرد بنجاح \n★︎  يمكنك ارسال (['..klma..']) لسماع البصمه الاتيه .')
elseif msg.content_.ID == "MessageAnimation" then
redis:hset(nk..'replay_animation:group:'..msg.chat_id_,klma,msg.content_.animation_.animation_.persistent_id_)
redis:del(nk..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه متحركه للرد بنجاح \n★︎  يمكنك ارسال (['..klma..']) لاضهار الصوره الاتيه .')
elseif msg.content_.ID == "MessageVideo" then
redis:hset(nk..'replay_video:group:'..msg.chat_id_,klma,msg.content_.video_.video_.persistent_id_)
redis:del(nk..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه فيديو للرد بنجاح \n★︎  يمكنك ارسال (['..klma..']) لاضهار الفيديو الاتي .')
elseif msg.content_.ID == "MessageAudio" then
redis:hset(nk..'replay_audio:group:'..msg.chat_id_,klma,msg.content_.audio_.audio_.persistent_id_)
redis:del(nk..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه للصوت للرد بنجاح \n★︎  يمكنك ارسال (['..klma..']) لاضهار الصوت الاتي .')
elseif msg.content_.ID == "MessageDocument" then
redis:hset(nk..'replay_files:group:'..msg.chat_id_,klma,msg.content_.document_.document_.persistent_id_ )
redis:del(nk..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه ملف للرد بنجاح \n★︎  يمكنك ارسال (['..klma..']) لاضهار الملف الاتي .')  
elseif msg.content_.ID == "MessageSticker" then
redis:hset(nk..'replay_sticker:group:'..msg.chat_id_,klma,msg.content_.sticker_.sticker_.persistent_id_)
redis:del(nk..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه ملصق للرد بنجاح \n★︎  يمكنك ارسال (['..klma..']) لاضهار الملصق الاتي .')
end  
...
sendMsg(msg.chat_id_,msg.id_,"★︎ تم تعيين الرابط بنجاح  ")
return false
end
...
sendMsg(msg.chat_id_,msg.id_,"★︎ تم تعيين كليشة الايدي بنجاح \n★︎ يمكنك تجربة الامر الان ")
return false
end
...
return sendMsg(msg.chat_id_,msg.id_,"★︎  تم الغاء الامر , يجب ان يحتوي رسالتك ع ارقام الصلاحيات المعروضه . \n")   
end
...
sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا البوت ليس لديه صلاحيه رفع مشرفين في المجموعه \n") 
elseif ResAdmin == '{"ok":false,"error_code":400,"description":"Bad Request: can\'t remove chat owner"}' then
sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا لا يمكنني التحكم بصلاحيات المنشئ للمجموعه. \n") 
elseif ResAdmin == '{"ok":false,"error_code":400,"description":"Bad Request: CHAT_ADMIN_REQUIRED"}' then
sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا لا يمكنني التحكم بصلاحيات المشرف مرفوع من قبل منشئ اخر . \n") 
...
return sendMsg(msg.chat_id_,msg.id_,"★︎ تم تعيين كليشة التعليمات بنجاح \n★︎ يمكنك تجربة الامر *{"..amr.."}* ")
end
...
if checknewamr  then
sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا لايمكن اضافه امر مكرر في القائمه \n...")
else
...
if checkAmr then
sendMsg(msg.chat_id_,msg.id_,"★︎  حسننا عزيزي , لتغير امر {* "..msg.text.." *} \n¦ ارسل الامر الجديد الان \n...")
redis:setex(nk..":firstAmrOld:"..msg.chat_id_..msg.sender_user_id_,900,msg.text)
else
sendMsg(msg.chat_id_,msg.id_,"★︎  عذرا لا يوجد هذا الامر في البوت لتتمكن من تغييره  \n")
end
...
if checkk ~=0 then
tiires =  "★︎  تم مسح الامر {* "..msg.text.." *} من قائمه الاومر \n..."
else
tiires = "★︎  هذا الامر ليس موجود ضمن الاوامر المضافه  \n"
end
...
sendMsg(msg.chat_id_,msg.id_,"★︎ تمت العملية بنجاح وتم تحويل ملكية البوت \n★︎ الى الحساب الاتي : ["..arg.USERNAME:gsub([[\_]],"_").."]")
...
sendMsg(msg.chat_id_,msg.id_,"★︎ جـيـد , الان لقد تم تفعيل الاشتراك الاجباري\n★︎ على قناتك : ["..msg.text.."]")
return false
end
else
sendMsg(msg.chat_id_,msg.id_," عذرا , عزيزي المطور \n★︎ هذا ليس معرف قناة , حاول مجددا .")
return false
end
end
...
Start_Bot() 
sendMsg(msg.chat_id_,msg.id_,"★︎  تم تغير اسم البوت  \n★︎  الان اسمه "..Flter_Markdown(msg.text).." \n")
return false
end
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اضافه كليشة الستارت بنجاح \n\n★︎ ملاحظه : كليشة الستارت للمطور الاساسي تكون ثابته اما لغير الرتب تكون حسب الي وضعتها')
end
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  هذا الرد ليس مضاف في قائمه الردود ')
else
...
return sendMsg(msg.chat_id_,msg.id_, "★︎  تم وضع الكليشه بنجاح كلاتي ★︎ \n\n*{*  "..Flter_Markdown(msg.text).."  *}*\n")
end
...
return sendMsg(msg.chat_id_,msg.id_,"★︎  تم وضع الترحيب بنجاح كلاتي \n" )
end
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  مرحبآ عزيزي\n¦ تم حفظ القوانين بنجاح \n★︎ ارسل [[ القوانين ]] لعرضها \n')
end
...
if data.ID == "Ok" then 
return sendMsg(msg.chat_id_,msg.id_,"★︎  تم وضع الوصف بنجاح\n")
end 
end,nil)
end
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اذاعه الكليشه بنجاح \n للمـجمـوعآت ⇠ *'..#groups..'* كروب \n للمـشـتركين ⇠ '..#pv..' مـشـترك \n')
end
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم اذاعه التوجيه بنجاح \n للمـجمـوعآت ⇠ *'..#groups..'* \n للخآص ⇠ '..#pv..'\n')			
end
...
if DelRd == 0 then 
return sendMsg(msg.chat_id_,msg.id_,'★︎  هذا الرد ليس مضاف في الردود العشوائيه ')
end
redis:del(nk..':caption_replay:Random:'..msg.text) 
redis:srem(nk..':KlmatRRandom:',msg.text) 
return sendMsg(msg.chat_id_,msg.id_,'★︎  تم حذف الرد بنجاح \n')
end
--=============================================================================================================================
...
if data.username_ then UserName = "@"..data.username_ else UserName = "احد المشرفين" end
sendMsg(arg.chat_id_,arg.id_,"¦ قام  ["..UserName.."]\n★︎  بتغير اسم المجموعه  \n★︎  الى "..Flter_Markdown(msg.content_.title_).." \n") 
...
welcome = (redis:get(nk..'welcome:msg'..msg.chat_id_) or "★︎ مرحباً عزيزي\n★︎ نورت المجموعة \n")
welcome = welcome:gsub("{القوانين}", redis:get(nk..'rulse:msg'..msg.chat_id_) or "★︎ مرحبأ عزيري ★︎  القوانين كلاتي ★︎ \n★︎ ممنوع نشر الروابط \n★︎ ممنوع التكلم او نشر صور اباحيه \n★︎ ممنوع  اعاده توجيه \n★︎ ممنوع التكلم بلطائفه \n★︎ الرجاء احترام المدراء والادمنيه \n")
...
GetUserID(msg.sender_user_id_,function(arg,data)
welcome = (redis:get(nk..'welcome:msg'..msg.chat_id_) or "★︎ مرحباً عزيزي\n★︎ نورت المجموعة \n")
welcome = welcome:gsub("{القوانين}", redis:get(nk..'rulse:msg'..msg.chat_id_) or "★︎ مرحبأ عزيري ★︎  القوانين  ★︎ \n★︎ ممنوع نشر الروابط \n★︎ ممنوع التكلم او نشر صور اباحيه \n★︎ ممنوع  اعاده توجيه \n★︎ ممنوع التكلم بلطائفه \n★︎ الرجاء احترام المدراء والادمنيه 😅\n")
...
sendMsg(v,0,"★︎   هناك شخص قام بالتعديل \n★︎   الاسم : ⇠「 "..NameUser.." 」 "..uuseri.."\n★︎  الايدي : `"..msg.sender_user_id_.."`\n★︎  رتبته : "..Getrtba(msg.sender_user_id_,msg.chat_id_).."\n★︎   نوع التعديل : "..Rwers.."\n★︎  المجموعة : "..Flter_Markdown((redis:get(nk..'group:name'..msg.chat_id_) or '')).."\n★︎   الرابط : "..redis:get(nk..'linkGroup'..msg.chat_id_).." \n" )
end
end
return sendMsg(msg.chat_id_,msg.id_,"★︎   نداء لمنشئيين : ["..usersmnc.."] \n★︎   هناك شخص قام بالتعديل"..uuseri.."\n★︎   الاسم : ⇠「 "..NameUser.." 」 \n★︎  الايدي : `"..msg.sender_user_id_.."`\n★︎  رتبته : "..Getrtba(msg.sender_user_id_,msg.chat_id_).."\n★︎  نوع التعديل : "..Rwers.."\n" )   
...
SendMention(arg.chat_id_,datau.id_,arg.id_,"★︎ العضو ⇠ "..USERNAME.."\n★︎  قمـت بتكرآر آكثر مـن "..arg.NUM_MSG_MAX.." رسـآلهہ‌‏ , لذآ تم تقييدك مـن آلمـجمـوعهہ‌‏ \n",12,utf8.len(USERNAME)) 
...
return sendMsg(msg.chat_id_,msg.id_,'★︎  لا يمكنني مسح الرساله المخالفه .\n★︎  لست مشرف او ليس لدي صلاحيه  الحذف \n ')    
end
...
SendMention(msg.chat_id_,data.id_,msg.id_,"★︎ العضو ⇠ "..USERNAME.."\n عذرا ممنوع اعادة التوجيه  \n",12,utf8.len(USERNAME)) 
...
SUDO_USER = redis:hgetall(nk..'username:'..SUDO_ID).username
if SUDO_USER:match('@[%a%d_]+') then 
SUDO_USERR = "\n★︎ راسل المطور للتجديد ["..SUDO_USER.."]"
else
SUDO_USERR = ""
end
if not ExpireDate and not msg.SudoUser then
rem_data_group(msg.chat_id_)
sendMsg(SUDO_ID,0,'🕵🏼️‍♀️¦ انتهى الاشتراك في احد المجموعات \n★︎ المجموعه : '..FlterName(redis:get(nk..'group:name'..msg.chat_id_))..'\n💂🏻‍♀️¦ ايدي : '..msg.chat_id_)
